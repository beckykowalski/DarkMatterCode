...
void RooPlotter(int binWidth, const char* datafile, int it) {
  ...
  RooGenericPdf sig("sig", "sig", "(1 + Sm*cos(0.0172*(x + phi)))", RooArgList(Sm, x, phi));
  ...
  RooFitResult *result = sumPdf.fitTo(*data, Strategy(2), Verbose(false), PrintLevel(0), Hesse(true), Save(true));
