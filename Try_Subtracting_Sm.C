...
void RooPlotter(int binWidth, const char* datafile, int it) {
  ...
  RooRealVar Ns("Ns", "Ns", 52000, 10000, 100000);
  RooRealVar Nb("Nb", "Nb", 52000, 10000, 100000);
  RooFormulaVar Ntot("Ntot", "Ns+Nb", RooArgList(Ns, Nb));
  double omega = 0.0172;
  ...
  RooAddPdf sumPdf("sumPdf", "sumPdf", RooArgList(bkg, sig), RooArgList(Nb, Ns));
  ...
  RooGenericPdf SmPDF("SmPDF", "SmPDF", "-Sm", RooArgList(Sm));
  RooAddPdf model("model", "model", RooArgList(sumPdf, SmPDF), RooArgList(Ntot, Ns));
  
  RooFitResult *result = model.fitTo(*data, Strategy(2), Verbose(false), PrintLevel(0), Hesse(true), Save(true));
  //  RooAbsReal *nll = extendedPdf.createNLL(*data);
  RooPlot *frame = x.frame();
  data->plotOn(frame);
  model.plotOn(frame), RooFit::LineColor(kBlue-7);
  model.plotOn(frame, RooFit::Components(bkg), RooFit::LineStyle(kDashed), RooFit::LineColor(kViolet+1));
  model.plotOn(frame, RooFit::Components(sig), RooFit::LineStyle(kDashed), RooFit::LineColor(kTeal+1));
